...
**Стратегическая цель: платный релиз в Steam (~$1, минимум Steam — $0.99).** Открытые вопросы: Steamworks-аккаунт ($100 за приложение, возврат после $1000 выручки), судьба GPL-3.0 и публичного репозитория (пользователь — единственный правообладатель, будущие версии можно релицензировать; уже выпущенные остаются GPL), витрина EN/RU, пайплайн загрузки билдов (steamcmd/depots).
