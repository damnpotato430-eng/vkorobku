...
**Стратегическая цель: платный релиз в Steam (~$1, минимум Steam — $0.99) как «поддержка автора».** Модель — двойное лицензирование: GitHub остаётся публичным GPL-3.0, Steam-сборка — под условиями автора (единственный правообладатель; внешние PR в Steam-сборку — только с CLA). v1 Steam-сборки = текущие бинарники без Steamworks SDK: автообновления через депо работают без SDK, DRM-free; SDK (Steamworks.NET) — только если понадобится оверлей/Cloud, за условной компиляцией, steam_api64.dll в публичный репо не коммитить. Осталось: Steamworks-аккаунт ($100, возврат после $1000 выручки), витрина EN/RU, пайплайн steamcmd/depots, опциональная EULA.
