...
v1 Steam-сборки = текущие бинарники без Steamworks SDK: автообновления через депо работают без SDK, DRM-free; SDK (Steamworks.NET) — только если понадобится оверлей/Cloud, за условной компиляцией, steam_api64.dll в публичный репо не коммитить. Осталось: Steamworks-аккаунт ($100, возврат после $1000 выручки), витрина EN/RU (страница должна висеть **≥2 недели** до релиза + ревью Valve 1–5 дней), пайплайн steamcmd/depots, опциональная EULA.
